...
function BreakdownTable({ title, subtitle, rows, nameKey='batch', highlight=false }) {
...
function SummaryBoard() {
  ...
  const ap127 = useM_s(()=>
    all.filter(f=>f.batch===HIGHLIGHT_BATCH).sort((a,b)=>a.date.localeCompare(b.date)||(minutesOf(a.start)-minutesOf(b.start)))
  ,[all]);
  ...
  return (
    <ArtboardShell style={{ display:'flex', flexDirection:'column' }}>
      ...
      {/* Body: scrollable */}
      <div style={{ flex:1, minHeight:0, overflowY:'auto', padding:'16px 24px 24px', display:'flex', flexDirection:'column', gap:20 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.1fr 0.9fr', gap:16, alignItems:'start' }}>
          ...
          {/* AP-127 spotlight */}
          <div style={{ background:'var(--surface)', border:'1px solid var(--line)', borderRadius:8, overflow:'hidden' }}>
            <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--line)', background:'color-mix(in oklch,var(--highlight) 8%,var(--bg-2))' }}>
              <div className="mono uc" style={{ fontSize:10,color:'var(--highlight)',fontWeight:600 }}>◆ AP-127 SPOTLIGHT</div>
              <div className="mono uc" style={{ fontSize:9,color:'var(--ink-3)',marginTop:2 }}>{ap127.length} FLIGHTS ACROSS ALL DATES</div>
            </div>
            <div style={{ maxHeight:280, overflowY:'auto' }}>
              {ap127.map(f=>{
                const { wd, mo, day } = fmtDay(f.date);
                const color = STATUS_COLOR(f);
                return (
                  <div key={f.id} onClick={()=>{ app.setDate(f.date); app.setDrawer(f.id); }}
                    style={{ display:'flex',gap:10,padding:'9px 16px',borderBottom:'1px solid var(--line-soft)',cursor:'pointer',alignItems:'center',
                    ...
                    <div className="mono uc" style={{ width:52,fontSize:9,color:'var(--ink-3)',flexShrink:0 }}>{wd} {String(day).padStart(2,'0')}</div>
                    <div className="mono num" style={{ width:44,fontSize:11,color:'var(--ink)',flexShrink:0 }}>{f.start}</div>
                    <div style={{ flex:1,fontSize:11,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{f.student||'—'}</div>
                    <div className="mono" style={{ fontSize:9,color:'var(--ink-3)',flexShrink:0 }}>{f.lesson}</div>
                    {f.isStandby&&<span className="mono uc" style={{ fontSize:8,color:'var(--col-stby)',padding:'1px 4px',border:'1px dashed var(--col-stby)',borderRadius:2,flexShrink:0 }}>STBY</span>}
